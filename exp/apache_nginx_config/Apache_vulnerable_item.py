...
def Check_directory_structure():
    res = os.popen("cat /opt/lampp/etc/httpd.conf | grep Options | grep -v '#' | grep -v 'None'").read().strip()
    if 'Indexes' in res:
...
def Check_http_method():
    ...
    res = os.popen("cat /opt/lampp/etc/httpd.conf | grep LimitExcept | grep -v '#'").read().strip()
    if len(res) == 0:
        print(
...
# 检查是否配置错误日志
def Check_error_log():
    res = os.popen("cat /opt/lampp/etc/httpd.conf | grep ErrorLog | grep -v '#'").read().strip()
    if len(res) == 0:
        print(
...
def Check_error_loglevel():
    res = os.popen("cat /opt/lampp/etc/httpd.conf | grep LogLevel | grep -v '#'").read().strip()
...
# 检查是否配置访问日志
def Check_visit_log():
    res = os.popen("cat /opt/lampp/etc/httpd.conf | grep CustomLog | grep -v '#'").read().strip()
    if len(res) == 0:
        print(
...
def Check_log_format():
    res = os.popen("cat /opt/lampp/etc/httpd.conf | grep LogFormat | grep -v '#'").read().strip()
    if len(res) == 0:
...
def Check_error_redirect():
    res = os.popen("cat /opt/lampp/etc/httpd.conf | grep ErrorDocument | grep -v '#'").read().strip()
    if len(res) == 0:
